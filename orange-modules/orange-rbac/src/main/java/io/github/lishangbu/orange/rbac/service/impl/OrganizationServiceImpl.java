...
public class OrganizationServiceImpl implements OrganizationService {
   ...
   *
   * <p>根据传入的分页参数和动态查询条件进行组织数据的分页查询，支持按 id、name、code、enabled、parentId 等字段进行过滤
   * 查询结果会被映射为 {@link OrganizationTreeNode}，便于前端直接构建组织树或展示层级信息
   *
   * <ol>
   ...
   */
  @Override
  public IPage<OrganizationTreeNode> getPageByOrganization(IPage<Organization> page, Organization condition) {
    ...
    // 查询所有顶级组织及其子孙节点
    organizations = organizationMapper.selectOrganizationsWithDescendantsByIds(rootIdPage.getRecords());
    IPage<OrganizationTreeNode> result = new Page<>(rootIdPage.getCurrent(), rootIdPage.getSize(), rootIdPage.getTotal());
    if (!CollectionUtils.isEmpty(organizations)) {
