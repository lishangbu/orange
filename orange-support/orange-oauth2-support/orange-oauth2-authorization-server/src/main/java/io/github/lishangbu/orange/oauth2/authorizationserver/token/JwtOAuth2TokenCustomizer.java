...
import io.github.lishangbu.orange.oauth2.common.constant.SecurityConstants;
...
public class JwtOAuth2TokenCustomizer implements OAuth2TokenCustomizer<JwtEncodingContext> {
  ...
  public void customize(JwtEncodingContext context) {
    ...
    if (context.getPrincipal().getPrincipal() instanceof OAuth2User user) {
      ...
      Object loginType = user.getAttribute(SecurityConstants.LOGIN_TYPE);
      if (loginType instanceof String) {
        // 同时检验是否为String和是否不为空
        claims.claim(SecurityConstants.LOGIN_TYPE, loginType);
      }
    }

    // 检查登录用户信息是不是UserDetails，排除掉没有用户参与的流程
    if (context.getPrincipal().getPrincipal() instanceof UserInfo userInfo) {
      // 获取申请的scopes
