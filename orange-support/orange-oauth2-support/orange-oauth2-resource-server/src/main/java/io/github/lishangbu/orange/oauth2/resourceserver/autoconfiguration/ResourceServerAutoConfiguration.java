 ...
 * 核心职责：
 * <ul>
 *   <li>配置受保护资源的请求匹配规则与认证要求</li>
 *   <li>禁用不必要的默认功能（如 csrf、formLogin、sessionManagement 等）以支持无状态的 API 场景</li>
 *   <li>为资源服务器设置统一的认证入口点和访问拒绝处理器，确保认证/鉴权失败时返回统一的 JSON 响应格式</li>
 *   <li>集成透明令牌（opaque token）验证逻辑，并确保当令牌无效时触发统一的异常处理</li>
 * </ul>
 *
 * 配置要点：
 * <ul>
 *   <li>使用 `@Order` 指定该 `SecurityFilterChain` 的优先级，避免与认证服务器链冲突</li>
 *   <li>优先调用 `oauth2ResourceServer(...)` 并在其内部显式设置 `authenticationEntryPoint`，以防资源服务器默认行为覆盖自定义的入口点</li>
 *   <li>通过 `http.exceptionHandling(...)` 提供全局的兜底处理器</li>
 * </ul>
 *
...
public class ResourceServerAutoConfiguration {
   ...
   * 配置说明：
   * <ol>
   *   <li>放行 `oauth2Properties.ignoreUrls` 中配置的 URL；其余请求都要求认证</li>
   *   <li>禁用 CSRF、CORS、表单登录、会话管理和记住我功能，以支持无状态 REST API 场景</li>
   *   <li>先配置 `oauth2ResourceServer(...)` 并显式设置 `opaqueToken` 与 `authenticationEntryPoint`，避免内置默认覆盖自定义入口点</li>
   *   <li>随后配置全局 `exceptionHandling`，设置统一的 `AuthenticationEntryPoint` 和 `AccessDeniedHandler`，用于处理未认证或无权限访问的情况并返回统一 JSON 响应格式</li>
   * </ol>
   *
  ...
  public SecurityFilterChain resourceServerSecurityFilterChain(HttpSecurity http) throws Exception {
    ...
    http.oauth2ResourceServer(
      oauth2ResourceServer -> {
        ...
        oauth2ResourceServer.authenticationEntryPoint(new DefaultAuthenticationEntryPoint())
